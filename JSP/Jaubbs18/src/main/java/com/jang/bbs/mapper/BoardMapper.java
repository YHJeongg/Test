...
public interface BoardMapper {
	...
	List<BoardVO> getBoardList(SearchVO searchVO); // 글 목록 조회
	
	BoardVO getArticle(int bno); // 글 조회
	int writeArticle(BoardVO board); // 글 저장
	int updateAricle(BoardVO board); // 글 수정
	void deleteArticle(int bno); // 글 삭제
	...
	void deleteReplyBybno(int bno); // 원 글에 소속된 댓글 전체 삭제
	...
	int addBoardLike(BoardLikeVO boardLike); // 등록글 추천 기록 저장
	int getBoardLike(BoardLikeVO boardLike); // 등록글 추천 기록 확인
