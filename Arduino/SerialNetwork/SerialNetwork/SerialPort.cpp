...
bool CSerialPort::OpenPort(CHAR * portname)
{
	CHAR fullName[MAX_PATH];
	strcpy(fullName, "//./");
	strcat(fullName, portname);
	m_hComm = CreateFile(fullName,
...
//		MessageBoxW(HWND_DESKTOP, L"GetCommState Error", L"Error", MB_OK + MB_ICONERROR);
		CloseHandle(m_hComm);
		return false;
	}
	...
	if(m_bPortReady == 0)
	{
		//MessageBoxW(HWND_DESKTOP, L"SetCommState Error", L"Error", MB_OK + MB_ICONERROR);
		printf("[ERROR: SetCommStateError\n");
		CloseHandle(m_hComm);
		return false;
	}
		...
		DWORD WriteTotalTimeoutMultiplier, DWORD WriteTotalTimeoutConstant)
{
	...
	if(m_bPortReady == 0)
	{
		//MessageBoxW(HWND_DESKTOP, L"StCommTimeouts function failed",L"Com Port Error",MB_OK+MB_ICONERROR);
		printf("[ERROR: StCommTimeouts function failed\n");
		CloseHandle(m_hComm);
		return false;
	}
...
bool CSerialPort::WriteByte(BYTE* bybyte, DWORD nSize)
{
	m_iBytesWritten=0;
	if(WriteFile(m_hComm, bybyte, nSize, &m_iBytesWritten, NULL) == 0)
		return false;
	else
		return true;
}
